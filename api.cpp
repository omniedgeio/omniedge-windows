...
API::API(QObject *parent) : QObject(parent) {
  ...
  this->baseURL = "https://api.omniedge.io/api/v2";
}
  ...
  connect(
      ...
      [=]() {
        ...
        if (status == ResponseStatus::Success) {
          ...
          auth.expiredAt = responseDoc["data"]
                               .toObject()["expired_at"]
                               .toVariant()
                               .toDateTime();
          this->userAuthSession = auth;
          QDesktopServices::openUrl(
              QUrl(responseDoc["data"].toObject()["auth_url"].toString()));

          connect(
              &authSessionWebSocket,
              QOverload<QAbstractSocket::SocketError>::of(&QWebSocket::error),
              [=](QAbstractSocket::SocketError error) {
                qDebug() << "API: Error when connecting to websocket" +
                                authSessionWebSocket.errorString();
              });
          ...
          connect(&authSessionWebSocket, &QWebSocket::textMessageReceived,
                  [this](QString message) {
                    if (bOnlyConnectOneTime) {
                      ...
                      this->refreshToken =
                          responseDoc["refresh_token"].toString();
                      qDebug() << "Token : " << this->currentToken;
                      this->getUserInfo();
                      this->registerDevice();
                      this->getVirtualNetworks();
                      emit token(Token{this->currentToken, this->refreshToken});
                    }
                  });
          QUrl wsUrl = QUrl("wss://api.omniedge.io/login/session/" + auth.uuid);
        ...
        qDebug() << "API: DONE Get Auth Session "
...
void API::registerDevice() {
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    qDebug() << "API: Done register device"
             << (status == ResponseStatus::Success);
    reply->deleteLater();
  });
}
...
void API::getRefreshToken() {
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    if (status == ResponseStatus::Success) {
      QJsonObject data =
          QJsonDocument::fromJson(reply->readAll())["data"].toObject();

    } else {
      emit error(status, reply->errorString());
    }

    qDebug() << "API: DONE Get Refresh Token "
             << (status == ResponseStatus::Success);
    reply->deleteLater();
  });
}
...
void API::getUserInfo() {
  QNetworkRequest networkRequest;
  networkRequest.setUrl(QUrl(this->baseURL + "/profile"));
  networkRequest.setRawHeader("Authorization",
                              "Bearer " + this->currentToken.toUtf8());
  QNetworkReply *reply = this->networkManager->get(networkRequest);
...
void API::refreshAccessToken() {
  QNetworkRequest networkRequest;
  networkRequest.setUrl(QUrl(this->baseURL + "/auth/refresh"));
  networkRequest.setHeader(QNetworkRequest::ContentTypeHeader,
                           "application/json");

  QJsonObject obj;
  obj.insert("refreshToken", this->refreshToken);
  QJsonDocument doc(obj);

  QNetworkReply *reply =
      this->networkManager->post(networkRequest, doc.toJson());
  qDebug() << "API: Refreshing access token...";
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    if (status == ResponseStatus::Success) {
      QJsonObject data =
          QJsonDocument::fromJson(reply->readAll())["data"].toObject();
      this->currentToken = data["token"].toString();
      this->refreshToken = data["refresh_token"].toString();
      emit token(Token{this->currentToken, this->refreshToken});
      qDebug() << "API: Token refreshed successfully";
    } else {
      qDebug() << "API: Token refresh failed";
      emit error(status, "Session expired. Please login again.");
    }
    reply->deleteLater();
  });
}
...
void API::getVirtualNetworks() {
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    if (status == ResponseStatus::Success) {
      QJsonArray data =
          QJsonDocument::fromJson(reply->readAll())["data"].toArray();
    ...
    qDebug() << "API: DONE Get virtual networks "
             << (status == ResponseStatus::Success);
    reply->deleteLater();
  });
}
...
void API::joinVirtualNetwork(QString uuid) {
  ...
  QJsonDocument data;
  QNetworkReply *reply =
      this->networkManager->post(networkRequest, data.toJson());
  ...
  connect(reply, &QNetworkReply::finished, [=]() {
    ...
    qDebug() << "API: DONE Join virtual networks "
             << (status == ResponseStatus::Success);
    reply->deleteLater();
  });
}

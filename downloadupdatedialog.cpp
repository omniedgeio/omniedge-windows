...
DownloadUpdateDialog::DownloadUpdateDialog(QWidget *parent)
    : QDialog(parent), ui(new Ui::DownloadUpdateDialog) {
  ui->setupUi(this);
  this->setModal(false);
...
void DownloadUpdateDialog::setupUi() { ui->button_install->setEnabled(false); }
void DownloadUpdateDialog::startDownload(QString version) {
#ifdef Q_OS_WIN
  tmpFile = new QFile(
      QDir::tempPath() + "/" + "omniedge-setup-" + version + ".exe", this);
  if (tmpFile->exists()) {
  ...
  QNetworkRequest downloadRequest(
      QUrl("https://connect.omniedge.io/install/download/" + version +
           "/omniedge-setup-" + version + ".exe"));
  QNetworkReply *r = netManager->get(downloadRequest);
  connect(r, SIGNAL(downloadProgress(qint64, qint64)), this,
...
void DownloadUpdateDialog::downloadFinished(QNetworkReply *reply) {
  if (reply->error() == QNetworkReply::NoError) {
    ...
    displayErrorMessage(tr("Error while downloading update. ") +
                        reply->errorString());
  }
}

void DownloadUpdateDialog::updateDataTransferProgress(qint64 done,
                                                      qint64 total) {
...
void DownloadUpdateDialog::handleSslErrors(QNetworkReply *reply,
                                           const QList<QSslError> &errors) {
  QString errorString;
  for (int i = 0; i < errors.count(); i++) {
    errorString.append(errors.at(i).errorString() + ", ");
  }
  WARNING(tr("Got SSL errors while connecting to ") +
          reply->request().url().host() + "." + errorString);
  displayErrorMessage(tr("Got SSL errors while connecting to ") +
                      reply->request().url().host() + "." + errorString);
}

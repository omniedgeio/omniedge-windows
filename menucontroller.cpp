...
MenuController::MenuController(QObject *parent) : QObject(parent) {
  ...
  connect(this, &MenuController::n2nConnectSignal, this->n2nWorker,
          &N2NWorker::connect);
  connect(this->n2nWorker, &N2NWorker::error, this, &MenuController::n2nError);
  connect(this->n2nWorker, &N2NWorker::connected, this,
          &MenuController::n2nConnected);
  connect(this->n2nWorker, &N2NWorker::disconnected, this,
          &MenuController::n2nDisconnected);
  ...
  connect(this, &MenuController::addTapInterfaceSignal, this->tapManager,
          &TapManager::addTap);
  ...
  connect(&this->api, &API::token, [=](Token token) {
    ...
    this->api.refreshToken = token.refreshToken;
  });
  connect(&this->api, &API::profile, [=](Profile profile) {
    userEmail = profile.email;
    emit updateEmail(userEmail);
  });
  connect(&this->api, &API::virtualNetworks,
          [=](QList<VirtualNetwork> virtualNetwork) {
  ...
  connect(&this->api, &API::connectInfo, [=](ConnectInfo connectInfo) {
    emit n2nConnectSignal(connectInfo.host, connectInfo.communityName,
                          connectInfo.secretKey, connectInfo.subnetMask,
                          connectInfo.virtualIP);
    emit updateStatus(tr("Status: Connecting..."));
    myVirtualIP = connectInfo.virtualIP;
  });
  connect(&this->api, &API::error,
          [=](ResponseStatus status, QString errorString) {
...
void MenuController::getUserIdToken(QString token) {
  ...
  if (settings.contains("refresh_token")) {
    this->api.refreshToken = settings.value("refresh_token").toString();
  }
...
void MenuController::n2nError(N2NWorkerError error) {
  if (error == N2NWorkerError::WINTAP) {
    ...
    emit showMessage(
        tr("Driver error"),
        tr("Installing tap adapter... Please retry after 15 seconds."));
  }
}

void MenuController::userInfoReply(ResponseStatus status, UserInfo info) {
...
void MenuController::virtualNetworksReply(
    ResponseStatus status, QList<VirtualNetwork> virtualNetworks) {
  if (status == ResponseStatus::Success) {
...
// void MenuController::superNodeInfoReply(ResponseStatus status, SuperNodeInfo
// info)
//{
//     if(status == ResponseStatus::Success){

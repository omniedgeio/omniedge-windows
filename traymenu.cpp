...
TrayMenu::TrayMenu() {
  ...
  // trayIcon->showMessage("OmniEdge", "Connect without concern",
  ...
  connect(disconnectAction, &QAction::triggered, controller,
          &MenuController::disconnectSN);
  ...
  connect(logoutAction, &QAction::triggered, controller,
  ...
  connect(updateAction, &QAction::triggered, updater,
...
void TrayMenu::createMenu(bool loginStatus) {
  ...
  if (loginStatus) {
    ...
    for (int i = 0; i < this->controller->virtualNetworks.length(); i++) {
      submenu =
          devicesMenu->addMenu(this->controller->virtualNetworks.at(i).name);
      ...
      connect(subAction, &QAction::triggered, [=]() {
        this->controller->joinVirtualNetworkManual(
            this->controller->virtualNetworks.at(i).uuid);
      });
      for (int j = 0;
           j < this->controller->virtualNetworks.at(i).devices.length(); j++)
        submenu->addAction(
            this->controller->virtualNetworks.at(i).devices.at(j).name);
    ...
    showMessage(tr("Login successfully"),
                tr("Bring intranet on the internet."));
  } else {
    statusAction->setVisible(false);
    ...
    showMessage(tr("Logout successfully"),
                tr("Looking forward to see you again"));
  }
}

void TrayMenu::dashboard() {
  QString link = "https://connect.omniedge.io/dashboard";
  QDesktopServices::openUrl(QUrl(link));
}

void TrayMenu::showAboutDialog() { aboutDlg->open(); }

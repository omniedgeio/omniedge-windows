...
void initWin32() {
  ...
  if( err != 0 ) {
    ...
    /* WinSock DLL.                                  */
    printf("INITIALISE_WINSOCK_2_X_FAILED");
...
int open_wintap(struct tuntap_dev *device,
  ...
  if(!DeviceIoControl(device->device_handle, TAP_IOCTL_GET_MAC,
    ...
    printf("GET_MAC_ADDR_FROM_TAP_FAILED<>%s<>%s\n",
   ...
   printf("OPEN_DEVICE<>%s<>%s<>%s<>%d<>%02X:%02X:%02X:%02X:%02X:%02X\n",
  ...
  if(system(cmd) == 0) {
    ...
    printf("SET_DEVICE_IP_FAILED<>%s<>%s\n",

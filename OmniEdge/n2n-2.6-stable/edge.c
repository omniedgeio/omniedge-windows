  ...
  case 'E': /* multicast ethernet addresses accepted. */
    {
      ...
      traceEvent(TRACE_NORMAL, "ETHERNET_MULTICAST_ENABLED");
      break;
    }
  ...
  case 'l': /* supernode-list */
    if(optargument) {
      if(edge_conf_add_supernode(conf, optargument) != 0) {
        ...
        traceEvent(TRACE_NORMAL, "TOO_MANY_SUPERNODE");
        exit(1);
      }
      break;
    }
  ...
  case 's': /* Subnet Mask */
    {
      if(0 != ec->got_s) {
        ...
        traceEvent(TRACE_NORMAL, "MULTIPLE_SUBNET_SUPPLIED");
      }
  ...
  default:
    {
      ...
      traceEvent(TRACE_NORMAL, "UNKNOWN_OPTION<>%c", (char)optkey);
      return(-1);
    }
  }
...
static int loadFromFile(const char *path, n2n_edge_conf_t *conf, n2n_priv_config_t *ec) {
  ...
  if(fd == NULL) {
    ...
    traceEvent(TRACE_NORMAL, "CONFIG_FILE_NOT_FOUND<>%s", path);
    return -1;
  }
  ...
  while((line = fgets(buffer, sizeof(buffer), fd)) != NULL) {
	...
	opt++;
      ...
      traceEvent(TRACE_NORMAL, "SKIPPING_UNRECOGNIZED_LINE<>%s", line);
...
static void daemonize() {
  ...
  traceEvent(TRACE_NORMAL, "PARENT_PROCESS_EXITING");
  ...
  if((childpid = fork()) < 0)
    ...
    traceEvent(TRACE_NORMAL, "DAEMONIZING_ERROR<>%d",
	       errno);
  else {
    if(!childpid) { /* child */
      int rc;
  ...
  traceEvent(TRACE_NORMAL, "ERROR_MOVING_DIRECTORY_/");
...
#endif
{
  ...
  if(called) {
    traceEvent(TRACE_NORMAL, "LEAVING");
    _exit(0);
  } else {
    //traceEvent(TRACE_NORMAL, "Shutting down...");
    traceEvent(TRACE_NORMAL, "SHUTDOWN");
    called = 1;
  }
...
int main(int argc, char* argv[]) {
  ...
  traceEvent(TRACE_NORMAL, "STARTING_N2N<>%s<>%s", PACKAGE_VERSION, PACKAGE_BUILDDATE);
  /* Random seed */
  srand(time(NULL));

  if(0 == strcmp("dhcp", ec.ip_mode)) {
    //traceEvent(TRACE_NORMAL, "Dynamic IP address assignment enabled.");
    traceEvent(TRACE_NORMAL, "DYNAMIC_IP_ENABLED");
    conf.dyn_ip_mode = 1;
  } else
  ...
  if(conf.encrypt_key && !strcmp((char*)conf.community_name, conf.encrypt_key))
    ...
    traceEvent(TRACE_WARNING, "COMMUNITY_AND_ENCRYPTION_KEY_SAME");
  ...
  traceEvent(TRACE_NORMAL, "STARTED");
